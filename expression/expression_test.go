...
func TestEvaluateExpressionWithVariables(t *testing.T) {
	...
	vLookup := func(key string) (any, error) {
		...
		return values.Get(kv.ParseNamespaceString(k)...), nil
	}
...
func TestBooleanOrderOfOpsThree(t *testing.T) {
	...
	vLookup := func(key string) (any, error) {
		...
		v := kvs.Get(kv.ParseNamespaceString(key)...)
		return v, nil
	}
...
func TestEvaluationEarlyBreakout(t *testing.T) {
	// we expect this expression that would normally generate a parsing error, to be skipped entirely
	// this creates less determinism in the code since not all control paths are evaluate, however it
	// enables the creator to only evaluate expressions that would otherwise break only when necessary
	// for instance, when the data is unavailable
	v, err := Evaluate("false && split('to', ' ')[3]", nil, nil)
	assert.NoError(t, err)
	assert.False(t, IsTruthy(v))
}

func TestEvaluationEarlyBreakoutFail(t *testing.T) {
	// we expect this expression that would normally generate a parsing error, to be skipped entirely
	// this creates less determinism in the code since not all control paths are evaluate, however it
	// enables the creator to only evaluate expressions that would otherwise break only when necessary
	// for instance, when the data is unavailable
	_, err := Evaluate("true && strip('hello')[99]", nil, fLookup)
	assert.Error(t, err)
}
