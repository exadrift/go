...
func TestStringSubcripting(t *testing.T) {
	v := `things:
  numbers: [1, 2, 3, 4, 5]
  words:
  - alpha
  ...
  num: 6`
	...
	s, err := FromMapping(m)
	assert.NoError(t, err)
	value := s.Get("things", "words", 1, -1)
	assert.Equal(t, "z", value)
}

func TestParseNamespaceString(t *testing.T) {
	v := `things:
  numbers: [[1, 2, 3], [2, 3, 5], [5, 6, 7, 8, 10, 11, 21, 22, 33, 44, 55]]
  words:
  - alpha
  - betz
  - gamma
  num: 6`
	...
	s, err := FromMapping(m)
	assert.NoError(t, err)
	value := s.Get(ParseNamespaceString("things.numbers[2][10]")...)
